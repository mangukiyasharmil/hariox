...
// Color validation to prevent XSS via style injection
const isValidCSSColor = (color: string): boolean => {
  ...
  if (/^hsl\(\s*[\d.]+\s+[\d.]+%\s+[\d.]+%\s*(\/\s*[\d.]+\s*)?\)$/.test(color)) return true;
  if (/^var\(--[a-zA-Z0-9-]+\)$/.test(color)) return true;
  const namedColors = ['red', 'blue', 'green', 'black', 'white', 'transparent', 'currentColor', 'inherit'];
...
const sanitizeColor = (color: string): string | null => {
  ...
  if (!isValidCSSColor(trimmed)) {
    console.warn(`Chart: Invalid color value rejected: ${trimmed}`);
    return '#000000';
  }
  return trimmed;
};
